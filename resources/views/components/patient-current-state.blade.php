...
<div>
    <div class="">
        <div class="row mb-4 justify-content-end">
            <div class="row">
                <button class="btn btn-primary text-sm mr-2" data-toggle="modal" data-target="#needTestModal">Need Test
                </button>
                <button class="btn btn-primary text-sm mr-2" data-toggle="modal" data-target="#needOTModal">Need
                    Operation
                </button>

                @if(empty($alreadyDischargeRequested->patientDischargeNew[0]))
                    <div type="button" class="btn btn-primary text-sm" data-toggle="modal"
                         data-target="#dischargeRequestModal">
                        Discharge Request
                    </div>
        ...
        <button id="addStatusBtn" class="btn btn-primary add-status-btn border {{ $patientStatus ? '' : 'mb-2' }}">
            Update Status or Add Medicine
        </button>

        <h2 class="h3 mt-2 pt-2">Patient Current Status</h2>

        <!-- Need Test Modal -->
        <div class="modal fade" id="needTestModal" tabindex="-1" aria-labelledby="needTestModalLabel"
             aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="h4" id="needTestModalLabel">Need Test</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('pathology.request') }}" method="POST">@csrf
                            <div class="row">
                                <input type="text" hidden value="{{ $patientLists->id }}" name="patient_id">
                                <input type="text" hidden value="{{ $doctor->id }}" name="duty_doctor_id">
                                <div class="col-12 form-group">
                                    <label for="patient-id">Patient ID</label>
                                    <input type="text" disabled class="form-control" id="patient-id"
                                           value="{{ $patientLists->patient_id }}">
                                </div>

                                <div class="col-12 form-group">
                                    <label for="test">Test Name*</label>
                                    <select class="form-control" id="test" name="test[]" multiple>
        ...
        {{-- Need OT Modal --}}
        <div class="modal fade" id="needOTModal" tabindex="-1" aria-labelledby="needOTModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="h4" id="needOTModalLabel">Need Operation</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('duty-doctor.reg-ot', ['id' => $patientLists->id]) }}"
                              method="POST">@csrf
                            <div class="row">
                                <input type="text" hidden value="{{ $patientLists->id }}" name="patient_id">
                                <input type="text" hidden value="{{ $doctor->id }}" name="duty_doctor_id">
                                ...
                                <div class="col-12 form-group">
                                    ...
                                    <input type="date" class="form-control" id="date" name="ot_date">
                                </div>

                                <div class="col-12 form-group">
                                    <label for="time">Operation Time*</label>
                                    <input type="time" class="form-control" id="time" name="ot_time">
                                </div>

                                <div class="col-12 form-group">
                                    <label for="assign-doctor">Assign OT Doctor*</label>
                            ...
                            <div class="row">
                                <div class="col-lg-3 col-md-4 col-sm-12 form-group">
                                    <button type="submit" class="btn btn-primary">Book OT</button>
                                </div>
                            </div>
                        </form>
                    </div>
        ...
        <script>
            $(document).ready(function () {
                $('#assign-doctor').select2({
                    ...
                    dropdownParent: $('#needOTModal')
                });
                $('#ot-name').select2({
                    placeholder: "Select Operation",
                    dropdownParent: $('#needOTModal')
                });
                // Select2 for test select
                $('#test').select2({
                    placeholder: "Select Test",
                    dropdownParent: $('#needTestModal')
                });
            });
        </script>

        {{-- Discharge Request Modal --}}
        <div class="modal fade" id="dischargeRequestModal" tabindex="-1" aria-labelledby="dischargeRequestModalLabel"
             aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3 class="h3" id="dischargeRequestModalLabel">Discharge Request</h3>
                        {{-- <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> --}}
                    </div>
                    <div class="modal-body">
                        Are you sure you want to request for discharge?
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">No</button>
                        <form action="{{ route('discharge-request', ['id' => $patientLists->id]) }}" method="POST">@csrf
                                ...
                                <table class="table status-table border">
                                    ...
                                    <tr>
                                        ...
                                        <td><input type="number" class="form-control"
                                                   name="temperature_input" placeholder="in °F"></td>
                                        <td class="text-center"><input type="checkbox" name="bp_normal"></td>
                                    </tr>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table status-table">
                                    ...
                                    <tr>
                                        <td><i class="fas fa-thermometer-half status-icon pr-2 text-primary"></i>Body
                                            Temperature
                                        </td>
                                        <td>
                                    ...
                                    <tr>
                                        <td class="d-flex justify-content-between">
                                            <div><i class="fas fa-pills status-icon pr-2 text-success"></i>Medicine
                                            </div>
                                            <div>
